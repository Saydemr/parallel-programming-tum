...
static void generate_test(unsigned int rocks_pos[][2], double rocks_vel[][2], double datapoints[][2], unsigned int seed)
{
    ...
    for (int i = 0; i < ROCKS_NUM; i++)
    {
        ...
        rocks_vel[i][0] = MAX_VELO * double(generator()) / double(std::minstd_rand0::max());
        rocks_vel[i][1] = MAX_VELO * double(generator()) / double(std::minstd_rand0::max());
        if (i < NUM_DATAPOINTS)
        {
            datapoints[i][0] = MAX_X * double(generator()) / double(std::minstd_rand0::max());
            datapoints[i][1] = MAX_X * double(generator()) / double(std::minstd_rand0::max());
        }
    }
}
